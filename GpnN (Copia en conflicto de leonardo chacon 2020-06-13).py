...
#-------------------------Solving the Master Equation System---------------------------
  ...
  def all_solutions_colors(self):
    #creating a list of colors based 
    # on how many diferents norms are
    sols = self.ODESols()[0]
    #merging all solutions in an array
    ...
    #converting to set because there would be repeated solutions
    all_unique_sols = set(all_sols)
    all_unique_sols_list = list(all_unique_sols)
    all_unique_sols_list.sort()
    
    start = 0.0
    stop = 1.0
    cm_subsection = linspace(start, stop, len(all_unique_sols_list)) 
    norms_color_set = [ cm.jet(x) for x in cm_subsection ]
    return [norms_color_set,all_unique_sols_list]
  ...
  def animate(self, boundary, time):
    ...
    name = 'G' + str(self.p) + '_' + str(abs(self.n)) + str(self.N)
    plt.title(name)
    #plt.text(2, 6, '$t =' + f"{time:.2f}" + '$', fontsize=30)
    ...
    #creating a list of colors based 
    # on how many diferents norms are
    all_sols_colors = self.all_solutions_colors()[0]
    all_sols = self.all_solutions_colors()[1]
    #print("sols",all_sols)  
    norms_color_set = []

    boundary_list = list(boundary)#converting boundary np.narray to list

    for i in boundary_list:
      index = all_sols.index(i)
      #print("index            ",index)
      norms_color_set.append(all_sols_colors[index])
    ...
    cm_aux = ListedColormap(all_sols_colors)#creating a color map
